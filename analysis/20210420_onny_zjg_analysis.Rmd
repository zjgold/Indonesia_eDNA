...
source(here("phyloseq-extended-master","graphical_methods.R"))
source(here("phyloseq-extended-master","tree_methods.R"))
source(here("phyloseq-extended-master","plot_merged_trees.R"))
source(here("phyloseq-extended-master","specificity_methods.R"))
source(here("phyloseq-extended-master","ternary_plot.R"))
source(here("phyloseq-extended-master","richness.R"))
source(here("phyloseq-extended-master","edgePCA.R"))
source(here("phyloseq-extended-master","copy_number_correction.R"))
source(here("phyloseq-extended-master","import_frogs.R"))
source(here("phyloseq-extended-master","prevalence.R"))
source(here("phyloseq-extended-master","compute_niche.R"))
source(here("phyloseq-extended-master","pairwise_adonis.R"))
source(here("phyloseq-extended-master","ggrare.R"))
library(here)
```
...
ASV.table_long_mpa <- readRDS(file=here("data","ASV.table_long_mpa.RDS"))
...
zjg_eidx <- readRDS(here("decontamination","Output_R","fish_12S_pre_occupancy_results_sum.taxonomy_tech_reps_summed_eDNA_index.RDS"))

zjg_reads <- readRDS(here("decontamination","Output_R","fish_12S_pre_occupancy_results_sum.taxonomy_tech_reps_summed_read_counts.RDS"))

zjg_asvs_reads_sites <- readRDS(here("decontamination","Output_R","fish_12S_pre_occ_site_ASV_reads_summed.RDS"))

zjg_asv_reads <- readRDS(here("decontamination","Output_R","fish_12S_pre_occ_site_asv_table.RDS"))

zjg_asv_index <- readRDS(here("decontamination","Output_R","fish_12S_pre_occ_site_asv_table_edna_index.RDS"))

zjg_asv_site_viz <- readRDS(file=here("decontamination","Output_R","fish_12S_pre_occ_site_ASV_reads_summed_4_viz.RDS"))

raw_asv_data <-readRDS(file=here("data","ASV.table_long_raw.RDS"))

metadata <- read.table(file=here("onny_metadata_11022020.txt"), header=T)

ASV.nested.mpa <- readRDS(file=here("data","ASV.nested_final_mpa.RDS"))

good_ASVs <- readRDS(file=here("decontamination","good_ASVs.RDS"))
```
...
mpa_detailed <- read.table(file =here("data","MPA","fishcard_12S_all_taxonomy_tables","fishcard_12S_all_ASV_taxonomy_detailed.txt"), header = TRUE, sep="\t")
