...
saveRDS(hash.key, file="hash.key")
...
# Cleaning Process 6: **Remove Stubbrn Contaminant Taxa** ----------------------------------------------------------------------------------------------------------------
...
ASV.nested$Step5.tibble[[2]] %>% 
  ...
  left_join(hash.key) %>% 
  filter(., str_detect(sum.taxonomy, c("Actinopteri","Chondrichthyes"))) %>%
  filter(., !sum.taxonomy %in% 
           c("Eukaryota;Chordata;Chondrichthyes;Rajiformes;Rajidae;Raja;Raja binoculata",
             "Eukaryota;Chordata;Actinopteri;Labriformes;Labridae;Semicossyphus;Semicossyphus pulcher",                    
             "Eukaryota;Chordata;Actinopteri;Salmoniformes;Salmonidae;Salmo;Salmo salar")) ->  good_ASVs
saveRDS(good_ASVs,file="good_ASVs.RDS")
...
hash.key <- readRDS(file="hash.key")
...
total_kept_taxa <- barcode_2_pre_df_taxa %>% unique()
...
combined_index <- decostand(cbind(barcode_2_prop), method = "max", MARGIN = 1)
####How both datasets are combined, on a common, comparable scale.

### Output Read Count Data
pre_results_reads = cbind(barcode_2_prop)
...
pre_results_reads %>%  colnames()
set_diff(colnames(pre_results_reads %>%  colnames()
))
